...
void WriteData::run() {
    ...
    if(!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) { // write error
        emit emit_write_data_res(tr("Write error, please check if the file is in use."));
        return;
    }
    if( file.write(data_str_.toStdString().c_str())) {
        emit emit_write_data_res(QString("%1: %2 ").arg(inst_name_, QString::number(data_str_.count("\n"))) + tr("records written successfully.")
                                 + QString(" (%1)").arg(file_name_));
